...
def check_block_stability(block1: _Block, block2: _Block) -> bool:
    ...
    if block1.vertexes.size == 0 or block2.vertexes.size == 0:
        return True

    goes_inside_block2 = {vertex.label : False for vertex in block1.vertexes}
    for vertex in block2.vertexes:
        for counterimage_vertex in vertex.counterimage:
            if counterimage_vertex in block1.vertexes:
                goes_inside_block2[counterimage_vertex.label] = True

    # all == True if for each vertex x in A there's a vertex y such that
    # x \in E({x}) AND y \in B.
    # not any == True if the set "image of A" and B are distinct.
    return all(goes_inside_block2.values()) or not any(goes_inside_block2.values())
